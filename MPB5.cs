...
namespace PokerBotApplication
{
            ...
            if (holeCards.Count == 2)
            {
                ...
                //{
                    ...
                    else
                    {
                        ...
                        int betAmount = BotOperations.WhatToBet(odds, int.Parse(tableChipsTxt.Text), int.Parse(opponentBetTxt.Text), int.Parse(myChipsTxt.Text), out float expectedValue);
                        if (!(expectedValue > 0)) {
                        ...
                        else
                        {
                            ...
                            {
                                MessageBox.Show($"You should raise {betAmount} for an Expected Value of +{expectedValue}$");
                            }
                        }
    ...
    public static class BotOperations
    {
        public static int WhatToBet(float odds, int tableChips, int chipOffer, int myChips, out float expectedValue)
        {
            ...
            for (int i = 0; i <= myChips; i++)
            {
                float currentEV = (odds * (i + tableChips)) - ((1 - odds) * i);
                Debug.WriteLine($"EV: {currentEV} POS: {odds * (i + tableChips)} NEG: { ((1 - odds) * i)}");
                if (currentEV > maxEv)
                {
                    maxEv = currentEV;
                    bet = i + chipOffer;
