...
namespace PokerBotApplication
{
    public partial class ManualPokerBot : Form
    {
        ...
        private void resultBtn_Click(object sender, EventArgs e)
        {
            ...
            if (holeCards.Count == 2)
            {
                ...
                {
                    PreFlopOperations.GetPreflopOdds(holeCards);
                }
                else if (communityCards.Count > 2 && communityCards.Count < 6)
                {
                    try
                    {
                        float odds = BotOperations.GetOdds(holeCards, communityCards);
                        Debug.WriteLine(odds);
                        string outputString = $"You should bet { BotOperations.WhatToBet(odds, int.Parse(tableChipsTxt.Text), int.Parse(opponentBetTxt.Text), int.Parse(myChipsTxt.Text), out float expectedValue)}";
                        outputString += $"for an Expected Value of {expectedValue}";
                        MessageBox.Show(outputString);

                    }
                    catch (FormatException)
                    {
                        MessageBox.Show("Incorrect input, did you put in chip values? ");
                    }
    ...
    {
        public static int MyCombinations;
        public static int WhatToBet(float odds, int tableChips, int chipOffer, int myChips, out float expectedValue)
        {
            ...
            {
                Debug.WriteLine($"EV:{(odds * (i + tableChips)) - ((1 - odds) * i)}, POS:{(odds * (i + tableChips))} NEG: { ((1 - odds) * i)} OTHER PLAYER ODDS {(1 - odds)}");
                if (((odds * (i + tableChips)) - ((1 - odds) * i)) > maxEv)
                {
                    maxEv = (odds * (i + tableChips)) - ((1 - odds) * i);
                    bet = i;
                }
            }
            expectedValue = maxEv;
