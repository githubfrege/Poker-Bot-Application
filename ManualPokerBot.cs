...
namespace PokerBotApplication
{
    ...
    public partial class ManualPokerBot : Form
    {
        ...
        private void resultBtn_Click(object sender, EventArgs e)
        {
            ...
            List<KeyValuePair<int, int>> communityCards = Parser.ToKvpList(holeCardsTxt.Text);
            if (holeCards.Count == 2)
            {
                MessageBox.Show(communityCards.Count.ToString());
                ...
                {
                    Debug.WriteLine(BotOperations.GetOdds(holeCards, communityCards));
                    MessageBox.Show("You should bet " + BotOperations.WhatToBet(BotOperations.GetOdds(holeCards, communityCards), int.Parse(tableChipsTxt.Text), int.Parse(opponentBetTxt.Text), int.Parse(myChipsTxt.Text), out float expectedValue) + "for an Expected value of " + expectedValue);
                    label2.Text = "hello";
                }
                else
                {
